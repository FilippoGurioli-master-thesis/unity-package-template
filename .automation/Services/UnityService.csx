...
public static class UnityService
{
    ...
    public static void OpenProject(ProjectConfig config, bool batch = false)
    {
        ...
        var args = $"-projectPath {config.SandboxPath} -logFile {Path.Combine(config.SandboxPath, "unity_init.log")}";
        var unityEditorPath = Path.Combine(DetectUnityInstall(config),
        ...
        {
            Log.Error("Failed to open Unity project. Check the " + Path.Combine(config.SandboxPath, "unity_init.log") + " file for more details.");
            throw;
        }
    }
